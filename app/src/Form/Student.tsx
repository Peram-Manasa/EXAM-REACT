...
{
    try{
        const r= await  axios.put(`http://localhost:5000/data/${id.id}`,updateData);
        setData(r.data);
    }
    catch(err)
    {
          ...
          <FormProvider {...StudentFormMethod}>
                <form onSubmit = {StudentFormMethod.handleSubmit(StudentFormSubmit)}>
                    ...
                    <div className="form_footer">
                        <PrimaryButton type = 'submit' onClick ={StudentFormMethod.handleSubmit(StudentFormSubmit)}>
                            Submit 
                        </PrimaryButton>
                    </div>
                </form>
